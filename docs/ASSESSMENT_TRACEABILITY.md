...
| Current global weather for a specific city | `Weather - Current` intent → `POST /webhook` → strict city resolution + free OpenWeather Current Weather data |
...
| Use the linked OpenWeather API resource | The free OpenWeather Current Weather API is used in the zero-cost default; optional Geocoding and One Call paths remain available when configured (`src/weather-service.js`) |
...
- Country/state qualifiers are verified, low-confidence non-city entries are rejected, and the interpreted location is returned visibly.
...
- Caching reduces weather-provider usage and improves response time.
