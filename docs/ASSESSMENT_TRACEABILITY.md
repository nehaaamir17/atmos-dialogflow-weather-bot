...
| Current global weather for a specific city | `Weather - Current` intent → `POST /webhook` → geocoding + One Call current data |
...
| Use the linked API resource | OpenWeather Geocoding API and One Call 3.0 (`src/weather-service.js`) |
| Return response in Dialogflow Chat | Valid `fulfillmentText` and `fulfillmentMessages` WebhookResponse |
| Architecture: Dialogflow ↔ webhook ↔ OpenWeather | README Mermaid sequence and runtime implementation |
